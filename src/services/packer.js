...
function packageManagerPack(dir, destinationDir) {
  ...
  const { stdout } = execSync(`${packageManager} pack`, { cwd: dir });
  const packageFilename = stdout.replace(/\n$/, '');
  const packagePath = path.join(dir, packageFilename);

  fs.moveSync(packagePath, resultFilename);
...
export async function packageManagerUnpack(tgzFile, destinationDir) {
  if (!(await fs.pathExists(tgzFile))) {
    return [];
  }

  const tmpDir = (await tmp.dir()).path;
  await decompress(tgzFile, tmpDir);
  return move(path.join(tmpDir, 'package', '*'), destinationDir, { dot: true });
}
...
export async function shoutemUnpack(tgzFile, destinationDir) {
  ...
  if (await fs.pathExists(path.join(tmpDir, 'cloud.tgz'))) {
...
function hasExtensionsJson(dir) {
  return fs.pathExists(path.join(dir, 'extension.json'));
}
...
async function offerDevNameSync(extensionDir) {
  ...
  const cloudPackageJson = syncCloudComponent
    && getPackageJson(path.join(extensionDir, 'cloud'));
...
export default async function shoutemPack(dir, options) {
  ...
  return spinify(async () => {
    _.forEach(dirsToPack, (dir) => {
      packageManagerPack(dir, packageDir);
    });
