  ...
  fetch(url).then(res => {
    if (res.status === 200) {
      res.body.contentLength =
        parseInt(res.headers.get('content-length')) || null;
      return res.body;
    }
    return Promise.reject(new Error(`Response status code is ${res.status}`));
  });

const requestGet = request => url =>
...
module.exports = (cacheDirectory, urlToStreamPromise = httpGet) => {
  ...
  const download = (url, opts) =>
    mkdirp(pendingCacheDirectory)
      .then(() => {
        ...
        return openFsReadStreamWithSize(cachedFilePath).then(stream =>
          attachOnDataEvent(stream, opts.onData),
        );
      })
      .catch(err => {
        if (err.code !== 'ENOENT') {
  ...
  download.clear = url => {
    ...
    return asPromise(fs.unlink, filePath).catch(err =>
      err.code === 'ENOENT' ? Promise.resolve() : Promise.reject(err),
    );
  };

  download.toFile = (url, destPath, opts) =>
    download(url, opts).then(stream => writeStreamToFile(stream, destPath));
