...
export function addShortcut(
  extJson,
  { name, title, description, screenName, pagesNames },
) {
...
function createShortcutCreationQuestions({
  ...
  const when = ({ shouldCreateShortcut }) =>
    shouldCreateShortcut || !parentName;
  const shortcutsWithoutScreen = _.filter(
    screens,
    ({ name }) => !_.find(shortcuts, { screen: `@.${name}` }),
  );
  ...
  return [
    {
      ...
      message:
        'Create a shortcut (so that screen can be added through the Builder)?',
      when: () => parentName,
    },
    {
      type: 'input',
...
export function addShortcutForScreen(extJson, screen, shortcut) {
  getOrSet(extJson, 'shortcuts', []).push({
    name: shortcut.name,
...
export function linkSettingsPageWithExistingScreen(extJson, page, screenName) {
  const shortcut = _.find(extJson.shortcuts, { screen: `@.${screenName}` });
  if (!shortcut) {
    throw new Error(
      `Shortcut for screen ${screenName} does not exist so it cannot be linked with settings pages. ` +
